...
/*
 ...
 List of configured Gulp comments for executing the specific project task

 1: gulp --app app_patient
 2. gulp watch --app app_patient
 3. gulp watch_build -app app_patient
...
// list of projects
var projArray = ['app_patient', 'app_physician'];
...
var  hasApp = !!(projArray.indexOf(app) + 1);
if (!hasApp) {
  console.log(chalk.red('app [' + options.app + '] not exists.'));
  process.exit(1);
}
...
var paths = gulp.paths = {
  ...
  jsonFiles: [app + '/**/*.json', '!' + app + '/bower_components/**/*.json', '!' + app + '/i18n/**/*.json'],
  scssFiles: [app + '/*/styles/**/*.scss'],
  templates: [app + '/**/*.html', '!' + app + '/index.html'],
  cssFiles: ['.tmp/' + app + '/*/styles/*.css'],
  i18nFiles: [app + '/i18n/**/*'],
...
switch (paths.app) {
  case 'app_patient':
    ...
    paths.commonModule = [paths.common + '/**/*'];
    break;
}
